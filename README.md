...
**M0(코어 루프) + M0.1(영속화·취소) 동작.** 자연어 요청 한 건이 스냅샷 → 분류 → 모델 수정안 → 정책 판단 →
사용자 승인 → 파일 변경 → 결정론적 검증 → 최종 판정까지 완주하며, 모든 단계가 append-only 이벤트로 남는다.
앱을 껐다 켜도 그 기록이 남아 있고, 실행 중 취소는 **실제로 프로세스를 죽인다.**
...
- **최소 UI** — 워크스페이스 선택, Fast/Verified 정책, 단계 표시, 실시간 이벤트 로그, 승인 모달(실행될 argv 그대로 표시),
  diff, 검증 결과, 되돌리기, 최근 작업 목록.
- **실제 취소** — 취소 버튼이 실행 중인 자식 **프로세스 트리**를 죽이고(`npm test`가 띄운 `node`까지),
  진행 중인 공급자 HTTP 호출을 abort한다. "요청을 받아두고 다음 단계에서 확인"하는 협조적 취소가 아니다.
- **작업 영속화와 복구** — 작업 상태·이벤트·파일 변경·검증 결과가 SQLite에 남는다. 앱이 비정상 종료되면
  다음 실행 때 그 작업이 `INTERRUPTED`로 확정되고, 사용자가 되돌리기/다시 실행을 고른다(자동 재개하지 않는다).
...
M0.1은 `e2e-persistence.test.ts`가 같은 방식으로 검증한다 — 실제 60초짜리 명령을 취소해 **자식 프로세스의
PID가 실제로 죽었는지 확인**하고, 호스트를 SIGKILL로 죽인 뒤 새 프로세스가 DB를 열어 복구·롤백까지 확인한다.
...
- 멀티턴 세션, 다중 태스크 동시 실행
- 프로세스 샌드박싱 — 실행된 프로세스는 **스스로 추가 프로세스를 만들거나 파일을 바꿀 수 있고**, 취소는
  그때까지의 파일 변경을 되돌리지 않는다. Windows에서는 `taskkill /T`가 스냅샷 기반이라 종료 직전에
  새로 생긴 프로세스를 놓칠 수 있다(Job Object 미적용).
