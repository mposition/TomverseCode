...
그대로 적용된다 — 전환은 재시작의 축소판이다. 이 캐시는 아직 구현되지 않았고, 그것이 A를
"싸게" 만드는 데 남은 유일한 작업이다. **인덱스에는 자격증명이 없으므로** 살려두는 대상이
프로세스가 아니라 데이터일 때는 ①의 문제가 생기지 않는다.
