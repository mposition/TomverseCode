...
1. 승인 모달이 **여러 워크스페이스의 요청을 동시에** 다룰 수 있어야 한다(큐, 그리고 어느
   ...
   **화면 배선까지 끝났다**([ui-wireframes 3.30.2절](./ui-wireframes.md)). 붙이면서 드러난
   것은 "큐가 없다"가 아니라 **화면이 요청을 잃고 있었다**는 것이다: 승인 상태가
   `ApprovalRequest | null`이라 두 번째 요청이 첫 번째를 **덮었고**, 덮인 요청은 사라지는
   것이 아니라 호스트에서 타임아웃(10분)까지 매달리다 거부로 처리됐다. 그리고 응답 범위가
   "활성 워크스페이스 하나"로 남아 있었으므로, 그대로 뒀으면 **자기 Fleet의 승인이 전부
   거부됐을 것이다** — 구성원마다 게이트 루트가 다르기 때문이다.
...
2. 지출 표시가 **태스크 단위에서 세션 단위로** 올라가야 한다. 태스크당 상한은 그대로 두되
   ...
   **화면도 그 문장을 낸다**([ui-wireframes 3.30.3절](./ui-wireframes.md)). 다만 배선하려니
   기록이 답할 수 없는 질문이 하나 있었다: *"이 Fleet에 합계 상한이 걸려 있었는가."*
   `FLEET_ENROLLED`에는 예약 금액만 있었고, 그 상태에서 화면은 **"상한이 없었다"와 "상한을
   모른다"를 뭉갠다** — 정반대의 사실이다. 이제 등록 이벤트가 `caps`를 남기고, 그 객체가 없는
   옛 기록은 `capsRecorded: false`로 구별된다. 페이로드를 만드는 자리도 하나로 모았다
   (`fleet::Enrollment`) — 두 진입점이 각자 `json!`을 적으면 키가 갈라지고, 갈라진 쪽을 읽는
   화면은 한쪽 경로에서만 조용히 빈 값을 본다.
