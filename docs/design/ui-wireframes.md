...
### 3.10 최종 보고의 기준 체크리스트 (M1)

3.7절 검증 결과 패널 위에 **사용자가 정한 기준**이 먼저 온다. 지금은 build/test/lint 결과만 보고하므로 사용자가 무엇을 결정했는지가 최종 화면에서 사라진다(17.3절).
...
- 3.10절 기준 ↔ 테스트 연결의 표시 방식 — 어떤 테스트가 어떤 기준을 확인했는지 자동으로 잇지 못하는 경우가 대부분일 때 화면이 `?`로 뒤덮이는 문제
