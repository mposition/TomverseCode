...
`WORKSPACE_INDEX_CACHE_HIT`(적중, 원래 걸렸던 시간). 적중률과 회피된 시간이 쌓이면
"캐시가 필요한가"에 추정이 아니라 관측으로 답할 수 있다 — 그 답이 "아니다"로 나오면
지우는 것이 맞다.
