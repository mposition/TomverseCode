...
**그 기준으로 재봤더니 판정이 뒤집혔다**(16.7절). 심볼 층 이전 이 저장소의 구축은 약 **7 ms**
...
| `symbol-match` | 메시지의 식별자가 **`WorkspaceIndex.symbols[].name`과 정확히 일치**하는 파일 — Tree-sitter가 그 정의를 안다(16절) |
...
**근거의 강도와 순위가 뒤집힌다.** 고친 근거는 16.1절에 있다.

## 5. 심볼/의존성 인덱스

- **파서:** Tree-sitter — **구현됨(16절)**. 언어별 grammar를 MVP 범위(9절)만큼만 번들하며,
  네이티브 바인딩이 아니라 **WASM**(`web-tree-sitter` + `.wasm` grammar)을 쓴다. 그 선택의
  근거와 대가(속도·크기 실측)는 16.2절.
...
~~**이 표는 심볼 인덱스 층의 계획이고, 그 층은 아직 없다**(9절).~~ → **[16.6절](#166-증분-갱신--6절-표를-실제로-구현했다)에서 구현했다.**
...
- **깨진 중간 상태를 감추지 않는다.** 다시 읽은 내용이 문법적으로 깨져 있을 수 있다. 그래도
  ...
  (심볼 인덱스는 16절에서 생겼고, 이 규칙이 `symbolStatus === "parse-failed"`로 실제로
  돌고 있다 — 16.5절. 실측에서 파일 350개 중 1개가 여기 걸렸다.)
...
Tree-sitter grammar 및 스크립트 인덱싱(state-machine-and-protocol.md 8절)을 지원할 초기 범위: **JavaScript/TypeScript, Python, Rust** — 16절에서 구현됐다(`.tsx`/`.jsx`는 별도 grammar이므로 실제로 싣는 파일은 5개다). 이 세 언어는 프로젝트 자체 스택(Node sidecar, Rust core)과 겹쳐 dogfooding이 쉽고, Tree-sitter grammar가 성숙하다. Go/Java/C# 등은 이후 사용자 수요에 따라 추가. 범위 밖 언어의 파일은 `symbols`/`dependencyEdges` 없이 `fileTree`(경로·크기·언어태그)만 인덱싱되고, `mentioned`/`recently-changed` 판정은 가능하지만 `symbol-match`/`dependency`는 적용되지 않는다(텍스트 검색 폴백으로 보완).
...
비어 있고, 따라서 **`symbol-match`와 `dependency` 선정이 동작하지 않는다.**~~ → **16절에서 구현했다.**
다만 그때 지킨 규율은 그대로 남는다: **빈 결과를 만들어 "구현했다"고 보이게 하지 않는다.**
grammar를 못 실었거나 범위 밖 언어뿐인 워크스페이스에서는 `symbol-match`/`dependency`를 **여전히
아예 생성하지 않고**, 그 사실이 `symbolStatus`와 스냅샷 note에 남는다(16.5절).

~~6절 증분 갱신도 M0에서는 채택하지 않았다 — git 상태 지문이 바뀌면 전체 재구축한다.~~ →
**16.6절에서 구현했다.** 미뤄 둔 이유("증분 갱신은 심볼 인덱스가 있을 때 이득이 커진다")가
그대로 실현됐다: 이 저장소에서 전체 재구축이 7 ms에서 855 ms로 늘었으므로(16.7절), 지문이
...
- ~~Rust core와 Node sidecar 중 어디가 `WorkspaceIndex` 구축을 실제로 수행할지~~ → [process-architecture.md](./process-architecture.md) 6절에서 Node로 결정 (Tree-sitter npm 생태계 성숙도, 권한 불필요 작업은 Node에 두는 일관성, 대용량 데이터 프로세스 간 전송 회피가 근거). **16절에서 실제로 그렇게 구현했고, 세 근거 중 마지막이 조건부였다는 것이 드러났다**: 인덱스 자체는 Node 안에 머물지만 캐시 저장은 IPC로 Rust에 가므로 **payload가 커지면 그 왕복도 커진다**(93 KB → 1.03 MB). 지금은 IPC 한 줄 상한의 3%이고, 커지면 `p90PayloadBytes`가 먼저 말한다
- ~~5절 Tree-sitter 심볼/의존성 인덱스~~ → [16절](#16-tree-sitter-심볼의존성-인덱스-m3)에서 구현. **문서 두 곳이 서로 어긋나 있었다**(4절 표의 `symbol-match` 정의 vs 13.3절)는 것이 구현 중에 드러났고, 순위와 근거 강도의 관계로 판정해 **4절 표를 고쳤다**(16.1절). grammar는 네이티브가 아니라 **WASM**으로 싣는다 — 사용자 머신에 MSVC를 요구하지 않기 위해서다(16.2절). 남은 것: **들어오는 방향의 엣지**(누가 이 파일을 import하는가)와 **워크스페이스 패키지 해석**(`@tomverse/protocol` 같은 모노레포 내부 import)은 범위 밖으로 두었다(16.9절)
- **`SymbolEntry`를 그대로 저장하는 payload 형태** — 16.9절. 저장소가 10배면 payload도 10배이고, 압축하면 절반 이하로 준다. 하지 않은 이유는 "두 번째 모양"이 생기기 때문이며, 정할 근거는 `indexCache.p90PayloadBytes`다
- **파싱 실패율** — 이 저장소에서 350개 중 1개(16.7절)이고 원인은 grammar의 정규식/나눗셈 모호성이다. `metrics`의 `symbolIndex.filesParseFailedTotal`이 실사용에서 이 비율을 센다. 올라가면 고칠 자리는 추출 규칙이 아니라 **grammar 버전**이다
...
- ~~**Tree-sitter 심볼 그래프**(9절). `content-match`는 그 자리를 **메우는 것이지 대체하는 것이
  ...
  **[16절](#16-tree-sitter-심볼의존성-인덱스-m3)에서 해결.** 그리고 이 문단이 예고한 대로
...
- **덮은 수만 세고 앵커의 강도는 보지 않는다.** 정의 매치와 단순 등장 매치가 같은 무게다 —
  ...
  **16절이 재료를 바꿨다(답을 바꾸지는 않았다).** 이제 앵커의 출처가 셋이고 그중 하나는
...
### 16.1 4절 표의 `symbol-match` 정의가 13.3절과 어긋나 있었다 — 표를 고쳤다
...
### 16.2 grammar는 WASM으로 싣는다 (되돌리기 비싼 결정)
...
### 16.4 심볼의 종류는 프로토콜의 7종으로 접는다 — 접는 규칙을 한 곳에 둔다
...
### 16.5 "심볼이 없다"와 "파싱 못 했다"를 같게 만들지 않는다
...
### 16.7 실측 — 이 저장소에서 (2026-08, Linux 컨테이너)
