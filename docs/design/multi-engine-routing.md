...
`(providerId, modelId, transport, cliVendor?)`이고, **독립성 계산만은 `(providerId, modelId)`로
접는다** — 이 두 키가 다르다는 것이 21.7절의 핵심이라 여기 함께 적어 둔다.
...
**10.5절(출력 토큰 상한)과 직접 부딪힌다.** 공급자에 따라 추론 토큰이 출력 예산에서 나가므로,
상한을 고정해 두고 effort만 올리면 **답이 조용히 잘린다.** 잘린 patch는 "모델이 못 했다"로
읽히지 실패로 읽히지 않는다 — 10.5절이 "모델 최대치를 요청하지 않는다"로 정한 계산의
입력에 effort가 들어가야 한다.
...
**독립성 산술은 경로가 아니라 모델로 센다.** A를 xAI API의 Grok으로, B를 Cursor의 Grok으로
두면 `transport`가 다르니 "다르다"로 읽힐 여지가 생기는데, 그건 **같은 모델에게 두 번 묻는
것**이다. 이 절 첫머리가 `providerId`를 나누지 말라고 한 것과 **똑같은 실패가 축만 바꿔**
돌아온다. 중복 제거는 `(providerId, modelId)`에서 하고, 소스 검사가 그 키를 확인한다.
