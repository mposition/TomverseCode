...
| ~~**보류**~~ | ~~Qwen/Mistral/DeepSeek/xAI~~ | ~~어댑터당 실제 유지보수 비용 발생. 근거 데이터 없이 추가할 이유 없음~~ → **21절에서 뒤집힘.** xAI·GLM·Kimi를 추가한다. 근거는 "이 모델이 낫다"가 아니라 **독립성 산술**이다 — A/B/C 세 자리가 서로 다른 공급자를 요구하는데 실제 공급자가 셋뿐이고 그중 하나는 미실측이다(21.1절). 이 이유는 보류를 정할 당시 존재하지 않았다 |
| **보류** | 데이터 기반 자동 라우터 | 평가 데이터가 없음. 지금 만들면 "추측이 담긴 설정 파일"일 뿐 |
| ~~**보류**~~ | ~~planner/executor 분리 실행~~ | ~~표준 태스크당 LLM 호출 3회 = 스파이크가 이미 비싸다고 판정한 방향~~ → **state-machine 72절에서 뒤집힘.** 계획 단계가 standard 흐름의 앞머리가 된다. 호출이 느는 것은 맞으나 그 대가로 **patch 검수(`REVIEWING`)가 물러났고**(72.3절), 늘어난 호출이 사용자 승인 게이트 앞에 서므로 사용자가 비용을 보고 승인한다 |
...
```
A(계획) ≠ B(계획 검토) ≠ C(결과 검토),  그리고 C ∉ 구현자 공급자
  → 최소 3개. 등급별 배정으로 구현자가 갈리면 4개
  → **`executionMode` 기본값 `verified`는 계획자를 둘 부른다**(72.9절) → 기본 상태에서 4~5개
  → 현재 실제 공급자는 openai · anthropic · google 셋
  → 그중 google은 실측으로 확인되지 않았다(19절)
  → 72절의 흐름을 완주할 수 있는 사용자가 사실상 없다
```
...
| `accounting` | `metered` \| `subscription` | CLI 로그인 경로(21.7절)는 토큰 단가가 없다 |
...
전송 화면이 사용자에게 **실제보다 더 많은 것을 보장하게 된다** — 6.3절이 export의 `guarantees`
블록을 만든 것과 같은 걱정이다.

```
endpointRegion:        우리가 고른 접속점 — 값은 "international" 하나뿐
providerJurisdiction:  공급자 법인 관할 — 우리가 못 고르는 사실, 표시만 한다
```

앞은 정책이고 뒤는 사실이다. `TransmissionPanel`이 둘 다 적되 구별해서 적는다.
...
**사다리는 위에서부터 **처음 맞는 줄 하나**만 적용한다.** 여러 줄이 동시에 성립할 수 있으므로
(공급자 2개 + 등급별 구현이 두 공급자에 걸치면 2행과 3행이 함께 참이다) 순서가 곧 우선순위다.
순서가 없으면 **원칙 4의 하드 불변식이 걸리는 바로 그 경우에 답이 둘**이 된다.

| # | 조건 | 배정 | 기록 |
|---|---|---|---|
| 1 | 구현자 공급자 밖에 독립 후보가 2개 이상 | A / B / C 전부 독립 | `resultReviewIndependent: true` |
| 2 | 구현자 공급자 밖에 독립 후보가 **1개뿐** | **C = B** (계획 검토자가 결과도 본다) | `result_reviewer_shares_provider_with_plan_reviewer` — 계획에 관여했으므로 **부분적 자기검토**임을 명시한다 |
| 3 | 구현자 공급자 밖에 독립 후보가 **없다** | **C를 드롭한다** | `result_review_dropped:no_independent_provider` |

**조건을 "가용 공급자 몇 개"가 아니라 "구현자 밖에 몇 개 남았는가"로 적는다.** 앞의 표현은
등급별 배정으로 구현자가 둘이 되는 경우를 세지 못했고, 그래서 2행과 3행이 동시에 참이 됐다.

**하드 불변식:** C는 이 태스크에서 코드를 쓴 어떤 공급자와도 달라야 한다 — 위 세 줄 모두 이
불변식을 **깨지 않는 범위에서만** 배정한다. 만족하지 못하면 같은 공급자로 "검증한 척"하지 않고
드롭한다(CLAUDE.md 원칙 4). **소프트 선호:** `C ≠ B`(2행이 그것을 포기하는 줄이다).

**드롭이 흐름을 막지 않는다.** 결정론적 검증은 이미 돌았고 그것이 최종 판정자이므로 C는 보조
신호다. 없으면 체크리스트가 짧아질 뿐이고, **짧아졌다는 사실이 체크리스트에 적힌다** —
말하지 않으면 사용자는 확인할 것이 적다고 읽는다.

#### C의 등급 요구는 `standard` 이상이다

C가 열린 범위의 코드 리뷰를 한다면 약한 모델은 잡음을 만들고, **잡음이 섞인 체크리스트는
사용자가 훑어 넘기는 법을 가르친다.** 그러나 C의 범위는 "승인된 계획과 일치하는가" 하나로
좁혀졌다(72.7절). 범위가 좁으면 요구 등급도 낮출 수 있다.

**frontier 선호, `standard` 허용, `economy` 불가.** 범위를 좁힌 직접적인 이득이 여기서 나온다 —
공급자 둘인 사용자에게도 C가 실제로 돌 수 있는 경우가 넓어진다.
...
- **비용은 0이 아니라 `unknown`이다.** 구독 정액이라 토큰 단가가 존재하지 않는다. 0으로 적으면
  예산 화면이 "안 썼다"고 거짓말한다 — 6.1절이 `resolved_model_id`가 NULL일 때 "대체 없음"으로
  접지 않은 것과 같은 규칙이다.
...
- **공급자 독립성은 무상관의 대리 지표일 뿐이다.** A/B/C의 가치 전부가 세 판단이 독립이라는 데
  ...
  대조가 계획 단계로 옮겨갔으므로(72.9절) 쌍별 데이터는 **계획 대조**에서 나온다 — 지표는
  공급자 쌍별 `doneCriteria`·`targetPaths` 불일치율이고, `executionMode` 기본이 `verified`라
  실사용에서 쌓인다. 공급자가 여섯이면 쌍이 열다섯이고, "어느 쌍이 실제로
...
- ~~**공급자 6개 × 역할 3개의 검증 조합**~~ → **전제가 틀렸다.** 어댑터는 역할을 모른다 —
  ...
  부분은 거기다.
- ~~**`grade`를 두 값으로 두는 것이 충분한가**~~ → **트리거를 적어 닫는다.** `balanced`는 tier가
  가르므로(72.9절) `economy`/`frontier` 둘로 충분하다. 셋째가 필요해지는 조건은 하나다:
