...
판정은 `tomverse-host windows-landing`이 한다(process-architecture.md 12절). 지금은 다섯 개가
전부 `not_implemented`다 — **실패가 아니다.** 그 둘을 같은 값으로 적으면 "만들었는데 깨졌다"로
읽히고, 다음에 할 일이 달라진다.
...
- **BYOK에서 공급자 6개 = 자격증명 6개일 때 Rust 쪽 Credential Store / 가용성 확인 UX** — **가용성 확인 절반은 17절에서 닫았다**(무료 조회로 자격증명을 확인하고, "조회된다"와 "호출된다"를 구별해서 말한다). **Credential Store 절반은 열려 있다**: 지금은 환경변수에서 읽으므로 데스크톱 사용자가 앱 안에서 키를 넣을 수 없다. 착지 기준은 Job Object(state-machine 20.6절)와 같은 모양이어야 한다 — Windows Credential Manager/DPAPI 연동은 **Windows에서 실행해야만 검증되고**, Linux 개발 환경에서 통과한 것은 그 코드에 대해 아무것도 말하지 않는다. 그때까지 화면은 "개발용 임시 방식"이라고 계속 말한다. **기준을 18절에 적었다** — 종전에는 "같은 모양이어야 한다"고만 적혀 있어서, 정작 Windows 앞에 앉은 사람이 무엇을 확인해야 하는지가 없었다
