...
> `simple`/`fast` 줄은 그대로다.
>
> | | 이 절이 쓴 때 | 72절 이후 (`standard`) |
> |---|---|---|
> | planner | 기본 비활성 | **흐름의 앞머리** — 항상 돈다(72.10절: frontier) |
> | 대조가 둘로 만드는 것 | executor | **계획자**(72.9절) |
> | patch 검수(reviewer) | 있음 | **물러났다**(72.3절) |
...
#### B의 독립성은 대조가 켜져 있으면 저절로 생긴다

B의 요구를 "A와 다른 공급자"가 아니라 **"살아남은 계획의 저자와 다른 공급자"**로 적는 이유가
여기 있다. 대조가 켜지면 계획이 둘이고, **사용자 승인 게이트에서 하나가 살아남는다**(불일치
카드가 그 게이트에 붙는다). 그 시점에 **저자가 아닌 쪽 계획자를 B로 쓰면** 자기 계획을 자기가
검토하는 일이 생기지 않는다.

이건 새 장치가 아니라 13.3절의 수법 그대로다 — 그쪽도 "살아남은 초안의 저자가 아닌 쪽"을
검수자로 쓰고, 그 확정을 배정 시점이 아니라 **살아남는 쪽이 정해지는 시점**으로 미뤘다.
여기서는 그 시점이 `AWAITING_PLAN_APPROVAL`이며, 라우터는 잠정 배정하고 오케스트레이터가
바꿔 낀다. 기록에는 `assignedPlanReviewer`와 `actualPlanReviewer`를 **둘 다** 남긴다.

**그래서 공급자 2개 + `verified`가 풀린다.** A = openai, A′ = anthropic, 사용자가 A의 계획을
고르면 B = anthropic이다. 셋을 요구하는 것처럼 보이던 자리가 둘로 선다.
...
**드롭은 흐름을 막지 않는다.** 결정론적 검증은 그대로 돌고, 무엇이 빠졌는지는 체크리스트와
`appliedPolicies`에 적힌다(state-machine 72.8절).
...
- **공급자 독립성은 무상관의 대리 지표일 뿐이다.** A/B/C의 가치 전부가 세 판단이 독립이라는 데
  ...
  — 지표는 공급자 쌍별 `doneCriteria`·`filesToChange` 불일치율이고(`targetPaths`가 아니다:
  그건 `PlanStep`의 필드라 이 경로에 없다 — state-machine 72.2.1절), `executionMode` 기본이
