...
**필수 필드가 아니라 `apiBaseUrl`과 같은 처리를 받는다**(21.4절). 즉 `transport`에 따라 타입을
갈라 **위 두 엔트리에는 필드 자체가 없다.** `"international"`로 채우지 않는다 — **없는 것은
없는 모양이어야 한다.** 이 절의 초안은 "필수 필드로 두고"라고 적었는데, 그러면 CLI 엔트리에
