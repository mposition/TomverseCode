...
- **`planner`** — ~~**기본 비활성.** 대규모 리팩터링에서 저비용 대용량 컨텍스트 모델이 먼저 계획만 세우는 용도. 역할 시스템에 *표현 가능하게* 두되 **지금은 켜지 않는다** — 표준 태스크당 LLM 호출이 3회가 되는데, 이는 스파이크가 이미 "쉬운 태스크엔 2회도 과하다"고 판정한 방향의 반대다. 실제 데이터로 큰 태스크에서 이득이 확인되면 그때 켠다.~~
  **→ `standard`에서 항상 활성이다**([state-machine 72절](./state-machine-and-protocol.md)).
  계획이 흐름의 앞머리가 되었고 **frontier 등급으로 돈다**(72.10절) — "저비용 대용량 컨텍스트
...
>
> 72절 이후 `standard`는 계획·사용자 승인 둘·등급별 구현·결정론적 검증·체크리스트까지를
...
> (72.8절). 불변식 자체(같은 공급자로 검증한 척하지 않는다)는 그대로다.
...
**③ `endpointRegion` — CLI 엔트리는 `apiBaseUrl`이 없다.** 21.5절의 필수 필드와 국내 호스트
denylist는 URL을 전제한다. CLI 엔트리에는 URL이 없으므로 **그 검사의 대상이 아니고**, 대신
`providerJurisdiction`만 표시한다. 여기서 중요한 것은 우리가 **접속점을 고르지 못한다**는
