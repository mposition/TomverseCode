...
### 10.5.1 `EffortLevel`이 들어갈 **자리** — 값은 아직 없다

21.4절이 남긴 요구를 여기 적어 둔다. **추론 토큰이 출력 예산에서 나가는 공급자가 있다면**
위 계산(*"모델 최대치와 `MAX_OUTPUT_TOKENS_PER_CALL` 중 작은 쪽"*)이 그 공급자에서 조용히
틀린다: effort를 올릴수록 추론이 답을 밀어내고, 남은 자리에 답이 안 들어가면 **출력이
잘린다.** 그 실패는 `schema_violation` → 재시도로 나타나므로 **원인과 멀다** — effort를
올렸더니 실패율이 오르는데 이유가 상한이라는 것은 보이지 않는다.

**그래서 자리만 비워 둔다.** `effectiveMaxOutputTokens`가 effort를 인자로 받을 자리이고,
지금은 받지 않는다:

- **어느 공급자가 그런지 확인하지 않았다**(21.4절이 "확인 날짜와 함께 적는다"고 정한 그 축).
  확인 전에 배수를 지어내면 10.5절이 애써 좁힌 상한이 근거 없이 다시 넓어진다.
- 확인하는 자리는 **14절의 적합성 스위트**다(아래). 거기서 "추론 토큰을 보고하는가"를 물으면
  이 질문이 자동으로 답해진다.

**확인되기 전까지 effort는 상한 계산에 들어가지 않는다.** 대신 상한은 그대로 걸리고, 잘림이
실제로 일어나면 `schema_violation` 재시도로 드러난다 — 드러나는 자리가 있다는 것이 자리를
비워 두어도 되는 근거다.
...
### 14.6 아직 이 스위트에 없는 것 — effort 두 항목

21.4절이 요구만 적고 자리를 비워 둔 둘이다. **여기 적어 두지 않으면 "적합성 스위트가 다
본다"로 읽힌다.**

| 물어야 하는 것 | 왜 이 스위트인가 | 답이 없으면 무엇이 막히나 |
|---|---|---|
| **effort 파라미터를 받는가** — 이름·단위·허용값 | 공급자마다 다르고, **확인 전에는 추측이다**(21.4절 규칙: 확인 날짜와 함께 적는다). 스위트는 실제 어댑터를 태우므로 물어볼 수 있는 유일한 자리다 | `ModelEntry.effort`가 `{ kind: "none" }`에 머문다 → 사용자가 고른 강도가 **아무 일도 하지 않고**, 그 사실만 화면이 말한다(72.4절) |
| **추론 토큰을 보고하는가** — 그리고 그것이 **출력 예산에서 나가는가** | 10.5.1절의 자리를 채우는 질문이다. usage envelope를 받는 것이 이 스위트의 본래 일이므로 항목 하나를 더하는 것으로 끝난다 | 출력 상한이 그 공급자에서 조용히 틀린다 → effort를 올릴수록 `schema_violation` 재시도가 늘고, **원인이 상한이라는 것은 보이지 않는다** |

**둘을 함께 둔다.** 앞을 확인하지 않으면 effort를 보낼 수 없고, 뒤를 확인하지 않으면 보낸
effort가 답을 잘라도 모른다 — 하나만 하면 나머지가 "나중에"로 남는데, 이 스위트의 14.5절이
바로 그 "나중에"가 조용해지는 것을 막으려고 만든 장치다.

**측정 순서는 등급(`grade`) 다음이다**(state-machine 72.16절). 등급이 없으면 effort 차이와
모델 차이가 섞인다.
...
| `accounting` | `metered` \| `subscription` | **비용 출처.** 지금 아는 CLI에서는 `transport: "cli"`와 `accounting: "subscription"`이 함께 가지만 **파생값으로 두지 않는다**(아래). 라우팅에서는 **같은 등급 안의 동점을 가르는 축**이지 등급을 바꾸지 않는다([state-machine 72.10.1절](./state-machine-and-protocol.md)) |
...
**단, 13.5절이 뒤쪽을 이미 하고 있다고 읽지 않는다.** 그 절의 규칙은 *"같은 모델 ID를 **두 executor
...
#### `accounting`을 `transport`의 파생값으로 두지 않는다

"CLI면 구독 정액"은 **오늘 우리가 아는 CLI들에 대한 관측**이지 법칙이 아니다. 사용량으로
과금하는 CLI가 하나만 나와도 깨지고, 그 표의 값은 **아직 확인하지도 않았다**(21.7절이 스스로
선언했다). 파생값으로 두면 그 순간 `transport: "cli"` + `accounting: "metered"`를 **적을 수가
없게** 되는데, 그건 사실을 타입이 막는 것이다.

**그래서 [state-machine 72.10.1절](./state-machine-and-protocol.md)의 옵트인 단서는
`accounting`이 아니라 `transport`로 건다.** "포함된 용량을 먼저 고른다"가 위험한 이유는
*구독이라서*가 아니라 **그 경로가 약관 리스크를 사용자에게 지우기 때문**이고(21.7절), 그
성질은 `transport: "cli"`에 붙어 있다. 조건을 관측에 걸면 **관측이 틀리는 날 단서가 조용히
빠진다.**
...
#### 이 축을 갖지 않는 엔트리가 둘 있다 — 이유가 서로 다르다

| 엔트리 | 왜 없는가 |
|---|---|
| `local://fake` | **네트워크로 나가지 않는다.** 접속점도 관할도 없다 |
| `transport: "cli"` | **접속점을 우리가 고르지 못한다.** 어디로 보내는지는 그 CLI가 정한다(21.7절 ③) |

둘 다 `"international"`을 달면 **그 값이 "확인된 국제 엔드포인트"를 뜻하지 않게 된다.** 한쪽은
확인할 것이 없어서, 다른 쪽은 확인하지 못해서인데 결과는 같다 — 값이 뜻을 잃는다.

**면제 근거를 "URL이 없어서"로 적지 않는다.** 국내 호스트 denylist가 URL을 전제하는 것은
맞지만 `endpointRegion`은 URL이 아니라 리터럴 타입 필드라 **URL 없이도 적을 수 있다.** 적을
수 있는데 적지 않는 이유가 위 표이고, 그 구별을 흐리면 "URL이 생기면 다시 걸면 되겠다"로
읽힌다.

**판정 축은 `providerKind`가 아니라 `transport`와 주소 스킴 둘이다.** `providerKindOf`는
`local://`로 fake만 가리고 **CLI를 가리지 못한다**(21.7절이 그 함수의 2값에 CLI를 접지 않기로
했으므로 아직 셋째 값이 없다). "필수 조건은 real 엔트리에만 건다"를 그 함수 하나에 맡기면
CLI가 `real`로 읽혀 조건이 걸린다.

#### 강제는 두 겹으로

**① 타입** — `endpointRegion`의 값을 `"international"` 리터럴 하나로 고정한다. 다른 값을
**적을 수가 없다.** 나중에 정책이 바뀌면 그때 유니온이 되고, 그 변경이 diff에 드러난다.

**필수 필드가 아니다.** 이 절의 초안은 "필수 필드로 두고"라고 적었는데, 그러면 CLI 엔트리에
`"international"`이 박히고 **그건 이 절과 21.7절이 둘 다 거짓말이라고 못박은 값**이다.

**그런데 두 예외를 타입이 **둘 다** 가르지는 못한다.** 위 판정 축이 둘이었던 이유가 여기서
드러난다:

| 예외 | 무엇이 가르는가 |
|---|---|
| `transport: "cli"` | **타입이 가른다** — `apiBaseUrl`과 같은 분기(21.4절). 그 엔트리에는 필드 자체가 없다 |
| `local://fake` | **타입이 가르지 못한다** — `transport`는 `http`이고, fake를 가리는 것은 `apiBaseUrl`의 **주소 스킴**이라 런타임 값이지 타입 판별자가 아니다. **②가 맡는다** |

`transport` 하나로 적으면 fake 쪽 절반이 남는다. **`"international"`로 채우지 않는다 —
없는 것은 없는 모양이어야 한다**가 두 예외 모두에 걸리고, 그것을 지키는 장치가 둘로 나뉜다.

**② 소스 검사** — 두 가지를 본다. 알려진 국내 호스트가 `apiBaseUrl`에 나타나면 실패하고,
**`local://` 엔트리에 `endpointRegion`이 있으면 실패한다.** 뒤가 없으면 ①이 덮지 못하는
절반을 아무것도 잡지 않는다 — 이 절의 요점이 "그 값이 뜻을 잃는다"인데 두 겹 어느 쪽도
막지 못하는 상태가 된다.
...
|---|---|---|
| ① 같은 모델에 경로가 둘이 된다 | **중개 여부와 무관하다** — 어떤 CLI든 **우리가 HTTP로도 부르는 모델**에 닿으면 경로가 둘이다 | 그 조건을 만족하는 CLI가 하나라도 있으면 성립 |
...
즉 저 표가 틀린 것으로 밝혀져도 ②는 그대로 남고, ①은 조건이 한 겹 붙으며, ③만 빠진다.

**①에 조건이 붙는다는 것도 적어 둔다.** *"CLI 로그인이 있으면 무조건"*이 아니다 — 21.8절이
**CLI 경로만 있는 모델**을 따로 다루는 것이 그 반례다. 경로가 둘이 되려면 같은 모델을 HTTP로도
불러야 하고, 어느 CLI가 그런 모델에 닿는지는 위 표의 미확인 값에 달려 있다. 다만 **레지스트리
키를 경로로 바꾸는 결정 자체는 그 조건이 한 번이라도 성립하면 필요하고**, 성립하지 않으면 키가
넓어도 손해가 없다 — 그래서 결정은 확인을 기다리지 않는다.
...
- **`accounting: "subscription"`이면 비용은 0이 아니라 `unknown`이다.** 정액이라 토큰 단가가
  존재하지 않기 때문이다. **키가 `transport`가 아니라 `accounting`인 것이 중요하다** — 막으려는
  것은 "셀 수 없는 비용을 0으로 적는 것"이고 그 성질은 과금 방식에 붙어 있다(21.4절: 둘을
  파생 관계로 두지 않는다). 사용량으로 과금하는 CLI가 나오면 그 엔트리의 비용은 셀 수 있고,
  이 규칙이 그 사실을 덮어서는 안 된다. **옵트인 단서가 `transport`에 걸리는 것과 방향이
  반대인 이유가 여기 있다**: 약관 리스크는 경로에 붙고 비용 가시성은 과금에 붙는다. 0으로 적으면
