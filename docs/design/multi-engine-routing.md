...
| 가용 독립 공급자 | 배정 | 기록 |
|---|---|---|
| 3개 이상 | A / B / C 전부 독립 | `resultReviewIndependent: true` |
| 2개 | **C = B** (계획 검토자가 결과도 본다) | `result_reviewer_shares_provider_with_plan_reviewer` — 계획에 관여했으므로 **부분적 자기검토**임을 명시한다 |
| C가 구현자 공급자뿐 | **C를 드롭한다** | `result_review_dropped:no_independent_provider` |

**하드 불변식:** C는 이 태스크에서 코드를 쓴 어떤 공급자와도 달라야 한다. 만족하지 못하면 같은
공급자로 "검증한 척"하지 않고 드롭한다(CLAUDE.md 원칙 4). **소프트 선호:** `C ≠ B`.
...
A/B/C 독립성은 그대로 막혀 있다 — 추가한 이유가 독립성이었는데 그렇다. 그리고 5단계는 **API 키와
예산 승인이 필요한 사람 몫**이다(CLAUDE.md의 CI 표). 순서상 이 작업 끝에 **유료 측정 승인이 한
번** 필요하다는 뜻이고, 미리 적어두지 않으면 그때 놀라게 된다.
...
- **공급자 독립성은 무상관의 대리 지표일 뿐이다.** A/B/C의 가치 전부가 세 판단이 독립이라는 데
  ...
  특정 공급자에 대해 그렇다고 주장할 근거는 없다. 다만 **잴 수 있다**: 대조 실행(executor ×2)이
  공급자 쌍별 불일치율을 남긴다. 공급자가 여섯이면 쌍이 열다섯이고, "어느 쌍이 실제로
...
- **공급자 6개 × 역할 3개의 검증 조합.** 적합성 스위트가 자동이라 다행이지만 CI·e2e 시간이
  늘고, 오류·rate limit 형식이 공급자마다 다르므로 `retry.ts`가 공급자별 분기를 갖게 된다.
  **보류 결정은 이 부분에서 옳았다** — 달라진 것은 그 부채를 살 이유가 생겼다는 점뿐이다.
- **`grade`를 두 값으로 두는 것이 충분한가.** `economy`/`frontier` 둘로 시작하지만
  `PerformanceProfile`은 셋이다(`balanced`가 tier로 가른다). 셋째 등급이 필요해지는 시점은
  측정이 두 값으로 갈리지 않을 때이고, 그건 재보기 전에는 모른다.
