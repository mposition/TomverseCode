...
> **차단 사유가 해소됐다 — 그런데 항목은 그대로 남아 있었다.** Tree-sitter 심볼·의존성
> 그래프는 이제 JS/TS·Python·Rust에서 돈다. 이 문단이 적어둔 **유일한** 보류 근거가
> 사라졌는데도 항목이 열린 채였다. 후속은 13.4.3절이다.
...
**틀렸다.** 무료인 것은 맞지만 **그 세트가 이 축을 재지 못한다.**

게이트 fixture의 workspace는 전부 `package.json` + `public.test.js` + 소스 파일 하나의 3파일
구조이고, 파일 81개 중 import 문이 있는 것이 7개다. 즉 **fixture 사이에도 안에도 의존 엣지가
사실상 없다.** 29건 전부 `impactRadius`가 0~1로 나오므로 분산이 없고, 분산이 없는 축에 대해
"어려움→simple이 내려가지 않았다"는 결과는 **축에 대한 정보가 아니라 세트에 대한 정보다.**

**그러므로 위의 "내려가지 않으면 버린다"를 이 세트에 적용하면 안 된다.** 적용하면 쓸 만한
신호를 측정 불가를 이유로 기각하게 된다 — 13.4.1절이 경로 위험 축의 대가를 재면서 *"대가가
0인 것은 이 세트의 쉬운 태스크에 위험 경로가 없기 때문이기도 하다"*라고 스스로 적어둔 것과
같은 종류의 한계이고, 이번에는 그 한계가 이득 쪽에 걸린다.

**필요한 것은 실제 저장소에서 나온 라벨 붙은 태스크다.** fixture 세트는 "어려움/쉬움" 라벨을
공짜로 주는 대신 **합성 workspace**라는 대가가 있었고, 그 대가가 여기서 처음 드러났다.
...
- 계획 대조의 판정 가능한 필드는 `doneCriteria`와 **`PlanOutlineStep.files`**다.
  `targetPaths`는 `PlanStep`의 필드라 이 경로에 없다 — 같은 질문("어디를 고칠 것인가")에
  대응하는 자리는 `files`이므로 `DisagreementField`가 그것을 가리켜야 한다.

**`patch` 자리는 여전히 두지 않는다**(53.5절). 완료 기준은 서술이고 patch는 산출물이다 —
전자를 더한다고 후자를 더할 이유가 되지 않으며, 계획 모드가 아끼려는 토큰이 바로 후자다.

그리고 새 phase 넷은 **화면의 5단계 매핑**(`phaseToStage`, ui-wireframes 2절)에 자리가 없다.
매핑을 넘기면 화면이 새 단계를 그리지 못하고, 그건 사용자가 승인을 기다리는 단계에서 일어난다.
...
| `planRounds` | **2** | 계획을 다시 세우는 횟수 — 72.4의 수정 요청과 72.8의 거부 경로 2가 **같은 카운터를 쓴다** |
...
- **TRIAGE의 판돈이 커졌는데 잴 세트가 없다.** 오분류의 대가가 `FIX_LOOP` 1회에서 **사용자
  ...
  **남은 것은 그 축을 켤 근거이고, 그게 지금 없다**: 게이트 fixture는 3파일 합성 workspace라
  의존 엣지가 없어 이 축에 분산을 만들지 못한다. 그리고 엣지를 TRIAGE의 손에 쥐여주려면
  `WorkspaceSnapshot`/`WorkspaceIndex` 경계를 건드려야 한다. **근거 없이 켜지도, 못 잰다고
  버리지도 않는다.**
