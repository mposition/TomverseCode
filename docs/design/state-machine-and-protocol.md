...
승인 게이트**에 올린다. 추가 호출은 1회, 추가 마찰은 0이다.

**카드는 계획을 고르게 하는 것이 아니다.** 17.8①이 초안에 대해 정한 규칙이 계획에도 그대로
적용된다 — **살아남는 것은 primary이고 사용자 선택으로 고르지 않는다.** 사용자가 카드에서
하는 일은 갈린 쟁점에 답하는 것이고, 그 답은 `acceptanceCriteria`로 고정되어 이후 단계가
참조한다. 이 구별을 흐리면 "사용자가 B안을 골랐다"는 없는 동작을 설계에 그리게 된다.
...
`fixLoopRounds ≤3`이 서브태스크마다면 N=10에서 30회다.
...
덮이지 않게 하므로 기계는 그대로 돈다. 문제는 예산이다 — 구성원이 사람을 기다리는 동안
