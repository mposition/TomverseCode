...
17.5절이 tier와 실행 모드를 갈라놓은 자리에 세 번째가 들어온다. **셋은 서로 다른 것을 켠다.**
...
**`PerformanceProfile`의 기본은 `balanced`다.** 세 축 중 이것만 기본값이 적혀 있지 않았는데,
...
- `packages/protocol/src/registry.ts` — `EngineRole`/`RoleAssignment`가 **B와 C를 구별하지
  못하고**, `RoutingDecision.reviewerIndependent`가 검토자 둘의 드롭을 표현하지 못한다.
  요구와 그 근거는 [multi-engine 21.6절](./multi-engine-routing.md)에 있고, **모양은
  `activeRoles` 소비처의 파급을 보고 정한다.**
