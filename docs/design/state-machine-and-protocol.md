...
- **강제 포기 이후 남은 프로세스의 정리** — Windows에서는 위 Job Object 구현에 딸려 닫혔다(`ProcessGuard`의 Drop). **Unix에서는 여전히 열려 있다** — 프로세스 그룹은 핸들처럼 닫히지 않으므로 강제 포기 시점에 남은 그룹을 정리하려면 별도 동작이 필요하다. 그리고 Windows 쪽도 위와 같은 이유로 **아직 검증되지 않았다.** 20.4절에서 **답이 정해졌다**: Job Object를 쓰면 추적할 것이 없다(`KILL_ON_JOB_CLOSE` job의 핸들을 놓으면 커널이 전부 죽인다). 따라서 이 항목은 독립적으로 풀 문제가 아니라 위 Job Object 항목에 딸려 닫힌다 — "이미 우리 손을 떠난 프로세스를 추적한다"는 문제 자체가 사라진다
