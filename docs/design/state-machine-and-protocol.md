...
- **UI 선택 화면.** 화면에 경로 입력은 생겼지만(3.16.2절) 고를 수 있는 목록은 없다. 그리고
  화면이 생기면 **무엇이 적용됐는지** 보여야 한다 — 지금은 stderr 한 줄이 그 역할을 한다.
  목록을 만들려면 "스킬이 어디 사는가"를 먼저 정해야 하고, 그 답의 절반이 34절이다:
  **워크스페이스 안은 아니다.**
...
- **저장소 안의 스킬을 읽어서 제안하는 것**(34.3절). 설정은 35절에서 풀렸지만 **스킬은 아직이다** —
  등록이 아니라 태스크마다 고르는 것이라 같은 모양이 아니고, 승인된 사본이 사는 자리를 먼저
  정해야 한다(35.8절).
- **적용된 스킬을 화면이 보여주는 것**(26.6절 그대로). 지금은 stderr 한 줄이다.
...
- **스킬의 공유.** 34.3절이 이 절을 가리켰지만, 스킬은 등록이 아니라 **태스크마다 고르는
  것**이라 같은 모양이 아니다. 저장소의 스킬을 쓰려면 승인된 사본이 사는 자리(상태 디렉터리의
  스킬 보관함)를 먼저 정해야 하고, 그건 26.6절의 "UI 선택 화면"과 같은 질문이다.
