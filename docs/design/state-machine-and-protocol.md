...
- **기준↔테스트 연결의 커버리지** — 위 규칙은 사용자가 테스트 파일명을 적어준 경우만 잇는다. 실제 사용에서 그 비율이 얼마나 되는지 측정된 바 없고, 낮다면 3.10절 화면이 사실상 언제나 물음표다. 넓히는 방법(테스트 이름 매칭, 커버리지 데이터)은 전부 새로운 오탐 경로를 여므로 실측 없이 손대지 않는다
- **위치 충돌 규칙의 오탐률** — "사용자가 지목한 파일을 하나도 건드리지 않음"은 좁은 규칙이지만 여전히 문자열 대조다. 재요청을 유발한 충돌 중 실제로 잘못된 계획이었던 비율을 봐야 규칙을 조일지 풀지 정할 수 있다
