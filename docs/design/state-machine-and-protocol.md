...
- **UI 등록 화면.** 지금은 `--mcp-server 이름=프로그램[,인자...]`뿐이고 쉼표가 든 인자는
  등록할 수 없다. 설정 파일이 생기면 그때 넓힌다 — **못 하는 것을 조용히 추측해서 하지 않는다.**
- **도구 목록을 모델에게 보여주는 것.** `tools/list`는 구현했지만 프롬프트에 싣지 않았다.
  싣는 순간 스냅샷에 없던 내용이 공급자로 나가므로 **전송 투명성(7절)이 그것을 세야 한다** —
  그 배선 없이 먼저 싣지 않는다.
...
(`TaskHost::new`에서 `detect_commands`를 한 번 부르고 그 결과를 들고 있는다). 실행 중에
...
- **상한값의 근거**(27.4절).
- **UI에서 무엇이 넘어왔는지 보여주는 것.** 지금은 전송 화면의 한 줄이다.
