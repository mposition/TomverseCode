...
#### 그러면 분할이 Fleet에서 실제로 버는 것은 무엇인가

**태스크당 상한과 계획의 실제 비용 사이의 차액이다.** 승인 카드는 그 계획이 얼마인지 금액으로
말하고(72.4절), 그 금액은 거의 언제나 태스크당 상한보다 훨씬 작다. 승인 시점에 구현 몫을
**카드 금액으로 줄이면** 그 차액이 즉시 다른 구성원에게 열린다 — 분할 이전에는 그 돈이 태스크가
끝날 때까지 잠겨 있었다.

| 시점 | 합계 원장이 잡고 있는 것 |
|---|---|
| 구성원 입장 | 태스크당 상한 전부 (계획 몫 + 구현 몫) |
| **계획 승인** | 계획 몫 + **카드가 말한 금액** — 나머지는 다른 구성원에게 열린다 |

이 방향은 `reserve_implementation`이 노출을 **줄이기만 한다**는 뜻이기도 하다. 그래서 그
함수는 거절할 일이 없고, 72.12절이 거부한 결말(*"승인한 작업이 돈이 없어 멈춘다"*)도, 대기 중인
구성원들이 서로의 정산을 기다리는 **교착**도 생기지 않는다. 거절 경로가 생기지 않았다는 사실은
`packages/toolchain/test/fleetBudgetStaging.test.ts`가 소스에서 지킨다.

**카드가 금액으로 말하지 못하면 줄이지 않는다.** 환산되지 않은 배정이 섞인 카드
(`unpricedAssignments`)에서 모르는 부분을 0으로 보면, 그 순간 상한이 조용히 사라진다.
`estimatedCostUsd`가 있어도 그것이 전부가 아닐 수 있으므로 **두 값을 함께** 본다.
...
확정은 `FLEET_IMPLEMENTATION_RESERVED`로 기록된다 — 잡은 금액, 열어준 금액, 그리고 **카드가
금액으로 말했는가**(`priced`)까지다. 마지막 값이 없으면 "0이 열렸다"와 "줄이지 않았다"가
구별되지 않는다.
...
| **72.12절의 Fleet 쪽 절반** | `FleetBudget`이 예약을 계획 몫과 구현 몫으로 나누고, **구현 몫은 승인 카드 금액으로 확정된다**. 감시(`PlanApprovalWatch`)는 한 곳뿐이고 화면·헤드리스 두 루프가 같은 것을 쓴다 | **구현했다** — 아래 72.12.1절이 처음 쓴 안이 왜 틀렸는지를 적는다 |
...
- ~~**Fleet 합계 예산의 단계 분할이 아직 없다.**~~ → **72.12.1절에서 구현했다.** 두 루프를
  ...
  `packages/toolchain/test/fleetBudgetStaging.test.ts`가 소스에서 지킨다.
  **다만 답의 모양이 72.12절이 적은 것과 다르다**: 입장 시점에 계획 몫만 잡으면 합계 상한이
  `PLANNING_SHARE`만큼 느슨해지므로(실측: $10 상한에 구성원 10개), 입장 판정은 태스크당 상한
  전부를 그대로 보고 **승인 시점에 구현 몫을 카드 금액으로 줄인다.** 남은 질문 하나:
  `PLANNING_SHARE`(지금 0.25)는 측정값이 아니라 자리표다. 계획/구현 비용 비는 가설 게이트가
  낼 수 있는 숫자이고, 그 전까지 이 상수는 *잠긴 금액의 표시*만 가른다(판정에는 쓰이지 않으므로
  틀려도 상한은 깨지지 않는다).
