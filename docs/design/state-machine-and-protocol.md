...
- **vitest·jest.** `npm test` 뒤에 무엇이 도는지는 매니페스트가 정하므로 argv로는 알 수 없다.
  ...
  매니페스트를 해석하는 일이라 29.3절이 이미 지문으로 다루는 자리와 얽힌다.
...
## 55. UNC 워크스페이스 — 없는 실패를 지어내고 있었다 (M3)
...
### 55.2 판정은 구조로 한다 — 출력 문자열도, 실행 결과도 근거가 아니다
...
### 55.3 지원할 것인가 — (a)·(b)를 재고, (d)를 골랐다

6절이 남긴 선택지는 둘이었다: **(a)** UNC를 드라이브 문자로 매핑해 지원한다,
**(b)** 시작 시 명확한 이유와 함께 거절한다. 55.2절을 하고 나니 질문이 달라졌다 —
...
### 55.6 무엇이 Linux에서 확인되고 무엇이 안 되는가
...
Win32는 UNC cwd를 받아들이고 거부하는 것은 cmd.exe 하나라는 55.1절의 문장이 관측으로
...
다루지 않았다면 **장벽이 통째로 지나갔을 것이다** — 55.5절이 경계한 자리가 가정이 아니라
기본 경로였다.
