...
- **커밋 뒤 되돌리기의 의미** — 되돌리기는 파일 내용만 복원하고 커밋은 저장소에 남는다. 지금은 그 사실을 **문장으로 알릴 뿐**이다. `git revert`를 제안할지, 커밋 이전으로 되돌릴지는 사용자의 워크플로(공유된 브랜치인가)에 달려 있어 우리가 정할 수 없다 — 되돌리기 화면에서 선택지로 물어야 하는지 검토 필요
