...
- **모델이 요청하는 `python`.** 프롬프트는 `python -m pytest`라고 적는데, 모델이 그것을 그대로
  ...
  틀릴 수 있다.
- **`.csproj`/`pyproject.toml`이 지문에 없다.** 49.5절이 지문을 프로그램까지 넓혔지만
  **매니페스트 목록은 여전히 `package.json`·`Cargo.toml` 둘뿐**이다. dotnet은 이 절 이전부터
  그랬고, Python 선언 파일도 지금 빠져 있다. Python 쪽은 프로그램 지문이 더 강한 방어를 하고
  있어 급하지 않지만, 목록이 `detect_commands`에서 유도되지 않는다는 문제는 그대로다.
