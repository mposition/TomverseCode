...
**엣지는 이미 있다.** `DependencyEdge`가 `fromFile`/`toFile`을 들고 있으므로 역방향 집계는
같은 배열을 한 번 더 도는 일이다.
...
- 확인: `npm run gate:g:triage-calibration` — 라벨 붙은 29건에 production 규칙을 태운다.
  **모델을 부르지 않으므로 유료 호출이 0이다**(13.4절이 이 항목을 유료 대기에서 빼낸 이유 그대로).
- 켜는 규칙: 13.4.1절과 같은 **지배 관계**다 — 한쪽을 개선하고 다른 쪽을 악화시키지 않을 때만
  켠다. 교환이 필요하면 교환비를 정해야 하는데 그 비율을 정할 근거가 아직 없다.
- 내려가지 않으면 **버린다.** 축을 더했다는 사실이 축이 일한다는 뜻은 아니다.
...
순서가 뒤집혀 보이지만 그렇지 않다. 16절의 권한 관할이 그대로 적용된다.
...
모델이 판정하지 않고 쟁점만 발굴한다는 16절 그대로다.
...
| 수정 요청 | `OUTLINING` 재진입 (`reviseRounds` 상한 안에서) |
| 거부 | `REJECTED` |
...
- **공급자가 계획자와 다르다.** 5절 불변식의 적용 대상이 patch에서 계획으로 옮겨온 것이다.
...
4.1절 실측이 blind를 철회한 근거는 **수리 행동을 억제했기 때문**인데, C는 수리하지 않으므로
그 대가가 발생하지 않는다. 숨기는 것은 모델의 산문이고 보여주는 것은 사용자가 승인한 것이다 —
성질이 다르다.
...
1. 지적한 항목으로 **`FIX_LOOP` 재진입** (상한 안에서)
2. **계획으로 되돌아간다** — 승인이 무효화되고 다시 세운다
3. **변경을 되돌리고 종료**
...
| 상한 | 값 |
|---|---|
| `maxSubtasks` | 계획이 만들 수 있는 서브태스크 수 |
| 전역 `fixLoopRounds` | 서브태스크별 상한과 **별개로** 태스크 전체에 건다 |
...
10.7절의 예약 상태 머신(`opened → settled` / `opened → released`)은 그대로다. 예약이 둘로
...
- ~~**TRIAGE의 판돈이 커졌다**~~ → **13.4.3절에서 답을 정했다.** 오분류의 대가가 `FIX_LOOP`
  1회에서 **사용자 게이트 둘과 검토 둘을 건너뛰는 것**으로 커진 것은 그대로다. 달라진 것은
  처방이다: 제안했던 "크기 축"은 **같은 것을 다시 재는 축**이었다(파일 개수는 너비인데 어려움은
  깊이다). `impactRadius`로 바꿨고, 반증 조건과 켜는 규칙을 함께 적었다. 남은 것은 그 스윕을
  실제로 돌리는 일이며 **유료 호출이 필요 없다.**
