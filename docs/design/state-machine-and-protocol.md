...
- **`anchorCoverage`와 `testAttribution`은 여전히 집계되지 않는다.** 둘 다 이번 세션이 만든
  재는 장치이고(context-engine 15.3절, 54절) 스냅샷·리포트에 실려 이벤트에 남는다. 같은
  이유로 붙일 수 있지만, 그 둘은 **태스크당 여러 건**이라 분모를 정하는 일이 먼저다 —
  60.2절이 말한 대로 잘못된 분모는 표본 부족보다 나쁘다.
