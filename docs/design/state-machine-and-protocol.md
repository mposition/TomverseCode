...
>
> **"TRIAGE가 살아난다"는 이 절과 충돌하지 않는다.** 이 절은 `fast`에서 대조가 켜지는 것을
> 막았고, 72.9절은 `verified`가 TRIAGE 판정을 통째로 버리던 **기본값**을 문제 삼는다 —
> 같은 방향의 두 걸음이지 반대가 아니다.
>
...
**이 이동은 두 자리에 적혀 있다** — 17.5절(대조의 두 축 규칙)과 [multi-engine-routing.md
13.4절](./multi-engine-routing.md)(호출 수 표)에 각각 취소선과 전후 대조표를 두었다. 두 축을
요구하는 규칙 자체는 살아 있고 바뀐 것은 무엇이 둘이 되는가다.
