...
- **검수자가 이동을 거부하는 길.** REVISE는 patch만 고칠 수 있고 이동은 실행자의 초안 그대로
  실린다(`mcpCalls`도 같다). 게이트가 이동마다 사용자 승인을 요구하는 것이 그 자리의 backstop이다.
...
- **검수자가 삭제를 거부하는 길.** 44.9절의 이동과 같은 자리다. `revisedPlan`으로 열지 않기로
  한 것이 45.2절이므로, 열려면 **검수자용 별도 필드**가 필요하다. 게이트가 삭제마다 사용자
  승인을 요구하는 것이 지금의 backstop이다.
