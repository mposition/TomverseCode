...
- **UI 배선.** 지금은 헤드리스 호스트의 플래그와 하위 명령뿐이다. 화면에서 격리 실행을 고르는
  자리와 "결과가 어느 트리에 있는가"의 표시는 3장에 아직 없다.
- **고아 트리 회수.** 앱이 비정상 종료되면 트리가 남는다. `INTERRUPTED` 확정과 같은 자리에서
  `ours`를 훑어 목록에 올리는 것이 자연스럽지만, **자동으로 지우지는 않는다** — 그 안에 사용자가
  꺼내야 할 변경이 있을 수 있고, 22.6의 규칙이 여기에도 그대로 걸린다.
- **태스크 기록과의 연결.** 어느 태스크가 어느 트리에서 돌았는지는 지금 이벤트에 남지 않는다.
  Arena(M4)는 트리와 후보를 짝지어야 하므로 그때 필요해진다.
...
- ~~**도구 목록을 모델에게 보여주는 것.**~~ → 31절. 막고 있던 전송 집계 배선이 7.2절에서
  ...
  일어나지 않았다(31.1절).
- **서버별 도구 허용목록.** 지금은 등록된 서버의 모든 도구를 부를 수 있다(승인은 매번 받는다).
  31.8절에도 남아 있다 — 목록을 좁히면 **프롬프트에 실리는 집합도 함께 좁아져야 한다.**
...
- **UI 배선.** 지금 Autopilot에 닿는 길은 `tomverse-host --approve autopilot`뿐이다. 화면에는
  무인 실행 토글도, 무인 정지를 사용자에게 알리는 자리도 없다(ui-wireframes에 항목 추가 필요).
...
- **시간·횟수 상한.** 무인 실행에 "언제까지"가 없다. `--timeout-secs`는 있지만 그건 하네스의
  상한이지 제품의 정책이 아니다.
...
- **UI 등록 화면.** 지금은 `--hook <phase>=<프로그램>[,인자...]`뿐이고 쉼표가 든 인자는
  등록할 수 없다 — MCP와 같은 한계이고 같은 시점(설정 파일)에 함께 넓힌다.
- **훅에 컨텍스트를 넘기는 것.** 지금 훅은 인자를 받지 않으므로 "어느 태스크의 어느 phase인가"를
  모른다. 환경변수로 넘기는 것이 자연스럽지만, 그러면 **무엇이 넘어가는지가 전송 투명성의
  대상**이 된다(7절) — 그 배선 없이 먼저 넘기지 않는다.
- **워크스페이스별 훅 등록.** 지금은 실행 인자라 매번 다시 적어야 한다.
...
- **UI 선택 화면.** 지금은 `--skill <파일>`뿐이다. 그리고 화면이 생기면 **무엇이 적용됐는지**
  보여야 한다 — 지금은 stderr 한 줄이 그 역할을 한다.
...
- **워크스페이스 안의 설정을 읽어 제안하는 것**(29.1절). 팀 공유는 그 경로로만 가능하다.
...
- **서버별 도구 허용목록**(23.10절 그대로). 지금은 등록된 서버의 모든 도구를 부를 수 있고,
  막는 것은 매번의 승인뿐이다. 목록이 좁아지면 프롬프트도 함께 좁아져야 한다 — **보여주는
  집합과 부를 수 있는 집합이 갈리면 모델이 거부될 것을 요청한다.**
...
- **여러 라운드의 이득 측정.** `mcpRounds` 기본 1은 관례적 선택이며, 라운드를 늘리는 것이
  이득인지 아직 재지 않았다.
- **도구 결과의 크기 상한.** 지금은 서버가 준 것을 그대로 싣는다. 서버가 큰 응답을 주면
  프롬프트가 그만큼 커진다 — 파일에는 예산이 있는데 여기에는 없다(context-engine 5절).
