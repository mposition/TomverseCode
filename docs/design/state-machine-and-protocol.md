...
덮이지 않게 하므로 기계는 그대로 돈다. 다만 예산은 **예약된 채로 사람을 기다린다** — 합계 예산
예약이 게이트 앞에서 잠기는 시간이 길어진다. 지금은 관측만 하고 정책을 만들지 않는다(아래
미해결).
...
- **TRIAGE의 판돈이 커졌다.** 지금 `simple`/`standard`를 가르는 것은 사실상 `maxRelevantFiles: 1`
  하나이고 그 상수는 코드 주석이 스스로 "초안이며 튜닝이 필요하다"고 적어 두었다. 오분류의 대가가
  종전에는 `FIX_LOOP` 1회였는데 이제 **계획·검토·사용자 게이트 둘을 통째로 건너뛰는 것**이 된다.
  라벨 붙은 세트에서 어려운 태스크 24건 중 19건이 `simple`로 분류됐다는 실측이 있으므로(13.4절),
  크기 축 추가가 이 흐름의 선행 조건이다. TRIAGE는 모델을 부르지 않으므로 **기록된 근거만으로
  임계값을 사후 재계산**할 수 있다(`sweepThreshold`).
- **벽시계 지연.** 계획(frontier) + 사용자 대기 + 검토 + N개 구현 + 검증 + C + 사용자 대기다.
  "그냥 빨리 고쳐줘"를 `simple` 경로가 제대로 받아내야 하고, 그것을 가르는 것은 TRIAGE 하나다.
- **Fleet의 게이트 대기 중 예산 예약.** 구성원이 승인을 기다리는 동안 합계 예산이 잠긴다.
  정책을 만들기 전에 실제로 얼마나 잠기는지 관측한다 — 지금 정하면 관측 없이 정하는 것이다.
