...
- **모델이 다시 그리게 하는 것**(41.6절). 상한 있는 새 루프와 phase 전이가 필요하고, "거부는
  태스크를 끝낸다"는 규칙을 바꾸는 결정이다.
- **PowerShell 명령 의미 분석**(12.4절의 둘째 줄 본체). `Get-ChildItem`·`Remove-Item` 같은
  cmdlet은 아직 그냥 프로그램 이름이다.
