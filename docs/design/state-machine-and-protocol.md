...
- **`delete_file`이 계획에 실리는 길.** 곁가지로 발견한 것: `buildExecutionPlan` 호출부가
  `plan: []`을 넘기고 있어서 **`toolHint: "delete_file"` 분기가 실제로는 아무 때도 돌지
  않는다.** 즉 모델은 지금 파일을 지울 수 없다. 여기서 조용히 열지 않는다 — 삭제를 모델이
  요청할 수 있게 하는 것은 이 절과 별개의 결정이고, 그 결정에는 "무엇이 지워지는지 계획
  단계에서 보이는가"가 함께 와야 한다.
