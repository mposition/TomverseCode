...
- **Windows Job Object로 프로세스 트리 종료를 보장한다** — 위 항목에서 갈라져 나왔다(20절). 16.3절 `taskkill /T`의 스냅샷 한계와 아래 "강제 포기 이후 정리"를 **둘 다** 닫는다. 설계·난점·착지 기준은 20.3~20.6절에 있다. **이번에 코드를 넣지 않은 이유는 검증 경계다**(20.5절): 이 저장소의 개발 환경은 Linux라 Windows 코드를 실행할 수 없고, 이 자리에서 컴파일되는 Win32 코드가 틀리는 흔한 방식은 핸들 수명이며 그 결과가 "앱이 스스로 죽는다"일 수 있다
...
- **강제 포기 이후 남은 프로세스의 정리** — 지금은 PID를 알려주고 사용자에게 맡긴다. 20.4절에서 **답이 정해졌다**: Job Object를 쓰면 추적할 것이 없다(`KILL_ON_JOB_CLOSE` job의 핸들을 놓으면 커널이 전부 죽인다). 따라서 이 항목은 독립적으로 풀 문제가 아니라 위 Job Object 항목에 딸려 닫힌다 — "이미 우리 손을 떠난 프로세스를 추적한다"는 문제 자체가 사라진다
...
- **17.4절 blocking 판정 규칙과 랭킹 순서의 임계값** — 조정 근거가 될 지표를 낼 계측이 17.10절 ⑩에서 붙었다(`field`, `metrics`의 `cardAnswers.byField`). **비율 자체는 실사용 데이터가 있어야 나오므로 이 항목은 열려 있다.** 읽는 법: `pickedPrimary`가 많다고 그 질문이 쓸모없었다는 뜻이 아니므로 절대값이 아니라 **필드끼리의 비교**를 볼 것. 튜닝할 때 고칠 자리는 여전히 `DISAGREEMENT_FIELD_RANK` 한 줄이다. 남은 것: **blocking 판정 규칙 자체**(서로소일 때만 `targetPaths`가 blocking 등)는 이 계측이 답하지 못한다 — 묻지 않은 쟁점에는 답이 없기 때문이고, 그걸 재려면 비-blocking 쟁점도 한 번은 물어보는 실험이 필요하다
...
- **한 카드 질문 상한 4개의 근거** — 물을 수 있게 하는 계측이 17.10절 ⑨에서 붙었다(`cardPosition`·`optionRank`·`cardSize`, `metrics`의 `cardAnswers`). **값 자체는 실사용 데이터가 있어야 나오므로 이 항목은 열려 있다.** 답을 읽는 법: 선택지 1번은 언제나 primary 실행자의 값이라 한쪽으로 치우칠 수 있지만 그 치우침은 카드 안의 모든 자리에 똑같이 걸리므로, **자리에 따라 비율이 달라지는 것**만이 자리 때문이다. 남은 계측 공백: 질문당 머문 시간을 재지 않는다 — 그걸 재려면 편집 중인 입력을 추적해야 하고, 그건 17.11절이 하지 않기로 한 것과 같은 성질의 기록이다
...
`not_verified`(프로젝트에 검증 명령이 없음)에서도 커밋하지 **않는다.** "검증되지 않았다"를
...
### 20.5 왜 이번에 코드를 넣지 않는가 — 검증 경계

이 저장소의 개발 환경은 Linux이고, **Windows 코드를 실행할 수 없다.**

확인한 것: `windows-sys`는 `cargo check --target x86_64-pc-windows-msvc`로 **타입 검증이
가능하다**(격리된 크레이트에서 실제로 확인했다). 다만 `tomverse-core` 전체는 `rusqlite`의
bundled SQLite가 `lib.exe`를 요구해 그 방식으로 검사할 수 없다.

확인할 수 없는 것: **동작.** 그리고 이 자리에서 컴파일되는 Win32 코드가 틀리는 흔한 방식은
핸들 수명이며, 그 결과가 "취소가 안 된다"가 아니라 **"앱이 스스로 죽는다"**일 수 있다
(`KILL_ON_JOB_CLOSE`가 걸린 job에 우리 프로세스가 들어가면 그렇다). 취소 경로에 실행해 본
적 없는 `unsafe`를 넣는 것은, 이 저장소가 `cmd.exe /c` 우회를 거부한 것과 같은 이유로
받아들이지 않는다 — **구조를 확인할 수 없으면 추측해서 실행하지 않는다.**

### 20.6 착지 기준

Windows 기계에서 다음이 전부 참일 때 착지한다. 하나라도 확인하지 못하면 `taskkill` 경로를 둔다.
