...
지금은 그 map 위에 그 사실을 주석으로 적어 두었다. 파생시키지 않은 이유는 두 타입의 필드가
일대일이 아니기 때문이다(`budgetUsd`·`modelPins`처럼 TS에만 있는 것이 있다) — 그러나 **주석은
검사가 아니다.** 필드가 더 늘면 유도 가능한 검사로 바꿔야 한다.
