...
- **강제 포기 노출 시점(5초)의 근거** — 실측이 아니라 추정이다. 너무 짧으면 정상 취소 중에 탈출구가 떠서 불안을 만들고, 너무 길면 탈출구가 없는 것과 같다. 실제 취소 소요 분포(`CANCELLATION_REQUESTED` → terminal 이벤트 간격)를 보고 정할 것 — 그 데이터는 이미 이벤트 로그에 있다
- **강제 포기 이후 남은 프로세스의 정리** — 지금은 PID를 알려주고 사용자에게 맡긴다. 앱이 대신 정리하려면 그 PID를 계속 추적해야 하는데, 이미 우리 손을 떠난 프로세스를 추적하는 것은 프로세스 샌드박싱(Job Object) 항목과 같은 해법이 필요하다
...
- **충돌하는 revert의 처리** — 지금은 충돌이 **불가능한 경우에만** revert를 실행하고(그 커밋이 아직 HEAD이고 워킹 트리가 깨끗할 때), 아니면 사유를 보여주고 아무것도 하지 않는다. 충돌을 감수하고 진행할지 묻는 화면을 둘 수도 있지만, 그러면 `git revert --abort`까지 우리가 책임져야 하고 사용자가 그 승인을 거부하면 충돌 마커가 박힌 채로 남는다 — 그 상태의 UX를 설계하기 전에는 열지 않는다
- **커밋 단위** — 지금은 태스크 하나가 커밋 하나다. fix loop를 여러 번 돈 태스크도 마찬가지라 중간 시도는 이력에 남지 않는데, 그게 바람직한지(깨끗한 이력) 아닌지(무엇을 시도했는지 잃음)는 사용자 취향의 문제다
...
### 19.3 충돌이 불가능할 때만 revert한다

`git revert`가 충돌하면 저장소가 **revert 진행 중** 상태로 남는다. 거기서 빠져나오려면
`git revert --abort`가 필요한데 그것도 승인을 거쳐야 하고, 사용자가 거부하면 충돌 마커가 박힌
채로 남는다. **되돌리기를 눌렀는데 저장소가 더 나빠지는 것은 있을 수 없다.**

그래서 두 조건을 먼저 확인한다.

1. 그 커밋이 아직 `HEAD`인가 (`git rev-parse HEAD`)
2. 커밋한 경로들의 워킹 트리가 깨끗한가 (`git status --porcelain -- <경로들>`)

둘 다 참이면 tip 커밋을 되돌리는 것이므로 **정의상 충돌하지 않는다.** 아니면 아무것도 하지 않고
사유를 돌려준다 — 사용자가 직접 판단할 재료를 주는 편이 우리가 추측해서 이력을 건드리는 것보다
낫다. 두 조회 모두 읽기 전용이고 **Policy Gate와 Tool Runtime을 그대로 지난다**: Rust가 자기
편의로 게이트를 우회하기 시작하면 게이트의 의미가 사라진다.
