...
- **화면.** 지금은 CLI JSON이다. 화면의 손으로 적은 한 문장("검증에서 멈춥니다")을 이 목록으로
  바꾸는 것이 남았고, 그것이 ui-wireframes 5절 Autopilot 항목 ①의 나머지다. 계산은 Rust에
  있으므로 화면이 할 일은 배치뿐이다 — 그래도 **`humanOnly`를 "스위치를 켜세요"로 뭉개면
  안 된다**(3.16절과 같은 규칙).
- **MCP 등록을 반영한 미리보기.** 지금 CLI는 풀을 붙이지 않으므로 `mcp_call`이 "등록 밖 거부"로
  나온다. 등록이 있으면 규칙 이름이 바뀌지만 **무인에서의 결말은 바뀌지 않는다**(어느 쪽도
  진행되지 않는다). 결말이 같으므로 급하지 않지만, 규칙 이름을 보여주는 이상 정확해야 한다.
