...
import { SYMBOL_INDEX_VERSION } from "../src/context/symbolIndex.js";

function index(paths: string[], excluded: { path: string; reason: string }[] = []): WorkspaceIndex {
  return {
    workspaceId: "ws-1",
    gitHeadAtIndex: "main@a",
    // `symbolStatus`/`symbolIndex`는 22절이 뒤에 더한 필드다. 이 픽스처의 파일은 `language`가
    // `null`이므로 애초에 파서가 없다 — 그 사실을 그대로 적는다("인덱싱했다"로 접지 않는다).
    fileTree: paths.map((p) => ({
      path: p,
      language: null,
      sizeBytes: 10,
      sha256: "x",
      symbolStatus: "unsupported-language" as const,
    })),
    symbols: [],
    dependencyEdges: [],
