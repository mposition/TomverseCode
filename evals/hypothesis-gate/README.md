...
**실제 가설 판정: ❌ FAIL (2026-08-27 confirmatory).** 288/288 완주, 인프라 실패율 0.0%,
실지출 $8.21. 사전 등록 기준 9개 중 6개 충족, 3개 미달 — 교차검증이 최강 단일보다 통과율이
**15.3%p 낮고**, paired bootstrap 95% 신뢰구간이 **[−0.292, −0.042]로 전부 0 미만**이며,
p95 지연이 **4.49배**다.

**이 판정의 범위**: 약한 실행자의 초안을 독립 검수하는 *현재 파이프라인*이 최강 단일 모델보다
유리하지 않다는 것이다. 교차검증이라는 개념 자체에 대한 판정이 아니다 — Arm A(gpt-4.1)가
6.9%만 통과했고 교차검증 arm은 그 초안을 재생한다. 자세한 것은 아래 실행 기록.
...
### 2026-08-27 confirmatory 완료 — **정식 FAIL**

**288/288 완주.** 예약 288건 전부 정산, 미해결 0건, `records/events 일치`,
**인프라 실패율 0.0%**. 실지출 **$8.2085**(카드 상한 $415.90의 **2.0%**).

동결 커밋 `bf21158`, 판정 기준 해시 `a089e94b57fd97c4`, seed 1, 순차 실행.

#### 판정: ❌ FAIL

사전 등록 기준 9개 중 **6개 충족, 3개 미달**. 미달이 곧 FAIL 사유다.

| # | 기준 | 실측 | |
|---|---|---|---|
| 1 | 유효 fixture ≥ 24 | 24 | ✅ |
| 2 | fixture당 반복 ≥ 3 | 3 | ✅ |
| 3 | 교차검증이 최강 단일보다 ≥ +10%p | **−15.3%p** | ❌ |
| 4 | paired bootstrap 95% CI 하한 > 0 | **[−0.292, −0.042]** | ❌ |
| 5 | correction ≥ harm × 2 | 6 vs 0 | ✅ |
| 6 | 보안 카테고리 regression 0건 | 0건 | ✅ |
| 7 | 비용 ≤ 최강 단일의 2배 | 1.82배 | ✅ |
| 8 | p95 지연 ≤ 최강 단일의 2.5배 | **4.49배** | ❌ |
| 9 | 인프라 실패율 < 5% | **0.0%** | ✅ |

#### Arm 비교 (24 fixture × 3회 = 288건)

| Arm | 구성 | 분모 | oracle 통과 | 통과율 | 평균 비용 | p50 | p95 |
|---|---|---|---|---|---|---|---|
| A | OpenAI 단독 | 72 | 5 | 6.9% | $0.0247 | 23.1s | 41.6s |
| **B** | **Anthropic 단독** | 72 | **22** | **30.6%** | **$0.0178** | **16.0s** | **27.4s** |
| C | 교차검증 (informed) | 72 | 11 | 15.3% | $0.0324 | 21.4s | 122.7s |
| D | 교차검증 (blind) | 72 | 8 | 11.1% | $0.0392 | 22.0s | 144.5s |

**분모가 네 arm 모두 72로 같다.** P1에서 있었던 분모 불일치(모델의 스키마 위반이 인프라 실패로
세탁되어 분모에서 빠지던 것)가 해소됐고, 그래서 `schema_violation` 16건이 실패로 분모에 남았다.

paired 결과는 **승 1 / 패 7 / 무 16**.

#### 지표 — 공통 분모 정의 (PROTOCOL-V2-DRAFT §2.1)

유효 paired 평가 **N = 72** (기준선 = Arm B, 교차검증 = Arm C).

| 지표 | 값 |
|---|---|
| `rescueRate` (B 실패 → C 통과) | 3 / 72 = **4.2%** |
| `regressionRate` (B 통과 → C 실패) | 14 / 72 = **19.4%** |
| **`netQualityImprovement`** | **−15.3%p** |
| 검산 | C 통과율 15.3% − B 통과율 30.6% = −15.3%p ✓ |

항등식이 성립한다 — 공통 분모를 쓸 때만 두 비율의 차가 pass-rate delta가 된다.

조건부 비율(보조, **primary 계산에 쓰지 않는다**): `conditionalRescueRate` 6.0%,
`conditionalRegressionRate` **63.6%**. 두 값의 차는 pass-rate delta가 **아니다.**

> `conditionalRegressionRate` 63.6%은 "B가 통과한 22건 중 14건에서 C가 실패했다"는 뜻이지
> **"검수가 B의 작업을 망가뜨렸다"가 아니다.** C의 초안은 B가 아니라 Arm A(gpt-4.1)에서 온다.
> 이 비교는 파이프라인 대 파이프라인이며, 그것이 사전 등록된 비교다.

#### 검수 기여 — 같은 초안에 대한 순효과 (A ↔ C)

| 분류 | 건수 |
|---|---|
| `correction` (초안 실패 → 검수 후 성공) | **6** |
| `harm` (초안 성공 → 검수 후 실패) | **0** |
| `no_measurable_correction` | 5 |
| `ineffective` (초안 실패 → 검수 후 실패) | **61** |

**검수는 한 번도 망가뜨리지 않았다**(harm 0, 기준 5번 충족). 그러나 실패한 초안 67건 중
**61건에서 아무것도 바꾸지 못했다.** 검수는 안전하지만 이 세트에서 대체로 무력하다.

blind vs informed: verdict 불일치율 20.8%, **oracle 결과 불일치율 12.5%**.

#### 실패 분류 (288건)

`requirement_unmet` 207 · **oracle 통과 40** · `schema_violation` 16 · `incomplete_fix` 16 ·
`wrong_patch` 9. **인프라 실패 0건.**

#### 통계 방법

paired bootstrap은 **fixture 단위로 재추출**한다(`stats.ts`). 같은 fixture의 반복은 서로
독립이 아니므로 실행 단위로 뽑으면 유효 표본 수가 부풀고 신뢰구간이 실제보다 좁아진다 —
없는 유의성이 생긴다. 10,000회 재추출, 95% 신뢰수준.

#### 이 결과가 말하는 것과 말하지 않는 것

**Protocol v1의 결론**: 약한 실행자의 초안을 독립 검수하는 **현재 파이프라인**은 가장 강한
단일 모델보다 유리하지 않다. 통과율이 15.3%p 낮고, 신뢰구간 전체가 0 미만이며, p95 지연이
4.49배다.

**제품 전체에 대한 결론이 아니다.** 교차검증이라는 개념 자체가 무의미하다고 말할 수 없다.
Arm A가 72건 중 5건(6.9%)만 통과했고 C/D는 그 초안을 재생하므로, 이 결과가 지지하는 것은
**"약한 실행자 + 강한 검수자" < "강한 모델 단독"** 이다.

**다음 질문**: 강한 실행자와 다른 공급자의 독립 검수를 조합하면 품질·비용 대비 이득이 생기는가.
이건 사전 등록되지 않은 별개의 가설이며 [PROTOCOL-V2-DRAFT.md](./PROTOCOL-V2-DRAFT.md)가
그 지표를 **이 결과를 보기 전에** 고정해 두었다.

#### 살아남는 것과 보류하는 것

이 결과로 **보류**하는 것 — 교차검증을 기본 경로로 쓰는 기능, 모든 작업에서 reviewer를 자동
호출하는 UX, 교차검증 자체를 핵심 품질 주장으로 쓰는 것. 삭제가 아니라 **의존성 중단**이며,
`critical` 작업에 한정한 선택적 전략으로는 남는다.

결과와 무관하게 **살아남는 것** — Evidence Ledger, 실제 테스트 기반 판정, 공급자 독립 Model
Adapter, Unified Safety Kernel, 전송·비용 투명성, 작업 위험도에 따른 검증 모드, 재현 가능한
Agent Trace.

이번 실행이 그 방향을 뒷받침한다: **검수는 288건 중 0건을 망가뜨렸지만 61건에서 무력했고,
결정론적 oracle 판정은 288건 전부에서 작동했다.** 제품의 중심은 "두 모델이 서로 검토한다"가
아니라 **"위험에 맞춰 가장 신뢰할 수 있는 증거를 구성한다"** 에 두는 편이 이 데이터와 맞는다.

#### 비용 실측

| | 상한 | 실지출 | 비율 |
|---|---|---|---|
| P0 (8건) | $11.55 | $0.168 | 1.5% |
| P1 (96건) | $138.62 | $2.673 | 1.9% |
| **confirmatory (288건)** | **$415.87** | **$8.209** | **2.0%** |

건당 $0.0285. 게이트 G 전체(probe + P0 5회 + P1 5회 + confirmatory 2회)의 확정 지출은
**$12.9631**이다.
