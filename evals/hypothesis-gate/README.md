...
예약을 해제하지 않고 `BLOCKED_UNRESOLVED_RESERVATION`으로 멈췄다. **자동으로 정리하지 않았다.**
실제 과금 여부는 공급자 청구 내역으로만 확인된다 — 대상 창은 2026-08-27T07:49:21Z ~ 07:51:21Z,
공급자 anthropic, 모델 `claude-sonnet-5`.
...
1. **미해결 예약을 청구 내역으로 확인한다.** 확인 전에는 `p1-pilot` 디렉터리를 재개할 수 없다
   (`BLOCKED_UNRECOVERABLE_RECORDS`).
2. **`providerTimeoutMs`를 fixture 상한과 맞출지 정한다.** 120초 기본값이 180초 fixture보다
   먼저 걸린다. 96건·288건 규모에서는 이 취소가 반복될 것이고, 매번 미해결 예약이 남아 실행이
   멈춘다. 늘리는 것이 옳은지, 취소를 `not_dispatched`로 볼 수 있는 근거를 만들 수 있는지는
   별도 판단이 필요하다 — **이번 작업에서 임의로 늘리지 않았다.**
