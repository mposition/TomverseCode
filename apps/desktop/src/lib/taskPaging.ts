 ...
 *
 * **이미 읽은 뒤 페이지를 버린다.** 목록의 순서 자체는 이제 움직이지 않지만(3.8.1절 —
 * 정렬 키가 `created_at`이다), 새로고침은 각 행의 **내용**을 다시 읽는 것이 목적이다.
 * 옛 페이지를 남기고 1페이지만 갈아 끼우면 위쪽은 새 상태, 아래쪽은 옛 상태가 되어
 * 어느 시점에도 존재한 적 없는 목록이 된다.
 */
 ...
 *
 * ~~방어하지 못하는 것: 페이지 사이에 갱신된 작업이 위로 이동해 사라질 수 있다~~ →
 * 3.8.1절에서 닫혔다. 필요한 것은 스냅샷이 아니라 **움직이지 않는 정렬 키**였다.
 * 목록은 `created_at` 내림차순이고 그 값은 production 경로 어디서도 바뀌지 않으므로,
 * 페이지 사이에 무엇이 갱신되어도 행이 커서를 넘어 이동하지 않는다.
 *
 * 여전히 방어하지 **못하는** 것: 페이지 사이에 **새로 생긴** 작업은 이미 읽은 1페이지 위에
 * 놓여야 하므로 이 목록에 나타나지 않는다. 그건 사라지는 것과 달리 새로고침으로 곧 보이고,
 * 무엇보다 사용자가 방금 만든 작업이라 화면 다른 곳에서 이미 보고 있다.
 */
