...
- **프롬프트에 섹션을 하나 더하면 `transmissionClaim.test.ts`가 잡는다.** 프롬프트에 실리는
  것은 전부 공급자로 나가므로 `transmission.rs`의 셋 중 하나로 분류해야 한다(화면이
  설명하는가 / 우리 지시문인가 / 아직 세지 않는가). 좋은 검사이고, **모르고 만나면 원인과
  먼 실패로 읽힌다.**
- **`OUTLINING`을 두 경로가 공유하면 전이 그래프가 실재하지 않는 경로를 참으로 읽는다.**
  ...
  **종류별로 지날 수 있는 phase를 적고 간선을 유도한다**(`PHASES_BY_KIND`).
  → 경로를 나누면 불변식이 **약해지기도 쉽다.** 극단적으로 전부 가두면 도달 불가가 되어
  검사가 공허해지므로, "계획 경로에 변경 phase가 없다"와 "변경 경로에 `OUTLINED`가 **아예
  없다**"를 따로 확인한다.
- **`unmeasured`를 검토 자리에서 막으면 fake 공급자로 그 경로를 태워볼 수 없다.** 카탈로그의
  실제 공급자는 전부 `unmeasured`이기 때문이다(21.8절). fake 셋에 **fixture 등급**을 붙여
  풀었다 — 측정값이 아니고, 실제 공급자가 측정 없이 등급을 얻는 것은
  `registryAxes.test.ts`가 막는다. 라우터가 fake를 특별 취급하게 만드는 쪽이 더 나쁘다.
- **`Record<TaskPhase, …>`는 새 phase를 컴파일로 잡지만 `switch`의 `default`는 안 잡는다.**
  같은 변경에서 한쪽은 빨개지고 한쪽은 조용하다 — `apps/desktop/src/types.ts`의
  `phaseToStage`가 그랬고, 조용한 쪽의 증상이 **"승인을 기다리는 태스크가 완료로 보인다"**였다.
  화면이 더 좋은 소식을 말하므로 아무도 신고하지 않는다.
- **테스트가 옛 계약을 초록색으로 지키고 있으면 정정이 회귀처럼 보인다.**
  `callPlan.test.ts`의 *"verified는 실행자를 둘 부른다"*가 그랬다. 이름이 사실을 주장하므로
  고치는 사람은 자기가 무언가를 깨뜨렸다고 읽는다 — **커밋 메시지와 테스트 주석이 그 사실을
  적어야 한다.**
- **`json!` 매크로 안에 `/** */`를 쓰면 파싱이 실패한다.** Rust의 doc 주석은 **항목**에만
  붙는다. 표현식 안에서는 `//`를 쓸 것. 함수 **파라미터**도 마찬가지다.
- **python으로 소스를 조각내 편집할 때 영역을 인덱스로 잘라내지 말 것.** `s.index(A)`부터
  `s.index(B)`까지를 지우는 방식은 그 사이에 **나중에 삽입한 것이 들어 있으면 함께 사라진다** —
  실제로 오케스트레이터의 새 메서드 600여 줄이 한 번에 지워졌고, 타입 검사가 "메서드가
  없다"로만 말해 원인이 멀었다. 앵커 문자열로 치환하거나, 자를 영역의 **양 끝을 먼저 출력해
  확인**할 것.
- **`#[derive(...)]`와 구조체 사이에 다른 항목을 끼워 넣으면 derive가 엉뚱한 타입에 붙는다.**
  위 함정과 같은 뿌리다. 증상은 `cannot find attribute serde`처럼 **끼워 넣은 쪽이 아니라
  원래 구조체의 필드에서** 나온다.
- **주석이 구현을 대신하고 있는지 검사가 묻지 않는다.** 독립 검토가 이 저장소에서 잡은 P0
  셋이 전부 그 모양이었다: `revert_and_stop`은 "Rust가 되돌린다"고 적고 아무것도 되돌리지
  않았고, 게이트의 취소 탈출구는 취소 신호가 닿지 않았으며, 워크스페이스 지문은 찍어서
  기록만 하고 비교하지 않았다. **셋 다 초록색이었다** — 검사가 "무엇을 했는가"만 보고
  "그래서 무엇이 막히는가"를 보지 않았기 때문이다. 새 안전장치를 넣을 때는
  **그것이 없으면 실패하는 검사**를 함께 쓸 것.
- **`ExperimentControls`를 하나라도 지정하면 대조가 조용히 꺼진다.** `contrastRequested`가
  *"하네스에서는 명시적으로 켜지 않는 한 끈다"*이므로, `pipeline`처럼 **대조와 무관한 축**을
  하나 주는 것만으로 호출 수가 달라진다. 실측으로 e2e가 "대조 이벤트가 없습니다"로 죽었고,
  원인은 그 테스트가 새로 준 `--pipeline`이었다. 축을 더할 때 **그 축이 `experiment`를
  만들게 되는지**를 먼저 볼 것 — 켜는 입구(`--contrast`)는 만들되 끄는 입구는 만들지 않는다
  (끄는 것이 이미 기본값이고, 둘을 두면 "지정하지 않음"과 "꺼짐"이 구별되지 않는다).
- **지표를 더하면 `metrics.rs`가 "읽는 법"을 요구한다.** `every_metric_is_read_by_a_question_
  or_is_explicitly_exempt`가 `Metrics`의 모든 키를 훑어 `openQuestions`가 읽거나 면제 목록에
  이유가 있는지 확인한다. 그리고 **질문의 분모를 태스크 수로 잡지 말 것** — 그 지표와 무관한
  태스크가 분모를 부풀려 표본이 없는데도 쌓인 것처럼 보이고, `open_questions_start_out_
  unanswerable`이 그것을 잡는다.
...
- **예약을 단계로 나눌 때 "잡아 둔 금액"만 세면 상한이 조용히 느슨해진다.** `FleetBudget`의
  단계 분할을 문자 그대로 구현했더니(입장 시점에 계획 몫만 예약) **합계 $10 / 태스크당 $3에
  구성원이 10개** 들어갔다. 아직 오지 않은 구현 비용이 없는 것처럼 보이기 때문이고, 그건
  `fleet.rs` 머리말이 한 층 아래에서 이미 경계한 실수다. 증상이 고약한 이유: 상한이 **측정한
  적 없는 상수**(`PLANNING_SHARE`)만큼 느슨해지므로 그 상수를 조정하는 것이 곧 상한의 뜻을
  바꾸는 일이 된다. **판정과 잠금을 분리할 것** — 입장 판정은 두 몫의 합(= 태스크당 상한)을
  보고, 분할이 버는 것은 승인 시점에 **카드가 말한 실제 금액**으로 줄이는 데서 나온다.
  `the_aggregate_cap_is_never_exceeded_even_when_everyone_spends_the_maximum`이 잡았다.
- **소스를 검사하는 테스트가 주석까지 세면, 다음 사람이 검사를 통과시키려고 설명을 지운다.**
  *"이 루프는 `PLAN_APPROVED`를 직접 찾지 않는다"*를 확인하는 검사가 **그 사실을 설명하는 doc
  주석**에 걸렸다. 앞 함정("자기 자신을 센다")과 뿌리는 같지만 대가가 다르다 — 저쪽은 개수가
  어긋날 뿐이고, 이쪽은 **주석을 지우는 것이 가장 쉬운 고치는 법**이 된다. 검사 대상이 코드인
  검사는 줄 주석을 빼고 볼 것(`fleetBudgetStaging.test.ts`의 `codeOnly`).
- **같은 규칙을 여러 진입점에 손으로 지키면 언젠가 하나가 빠지고, 빠진 쪽은 성공을 돌려준다.**
  타임아웃 없는 사용자 게이트의 탈출구는 취소뿐이라 태스크를 취소하는 모든 경로가
  `PendingGates::cancel_waiting`까지 닿아야 하는데, 세 곳 중 **둘이 빠져 있었다**
...
- **집계 조회의 실패를 0으로 접으면 예산에 자리가 열린다.** `task_cost_usd`를 못 읽었을 때
  ...
  예산에서는 **상한을 지키는 방향으로 틀리는 것**이 답이다(예약만큼 썼다고 친다). 추정이라는
  사실을 값으로 남기지 않으면 화면이 추정을 집계로 읽는다.
  **그리고 상태는 보통 둘이 아니라 셋이다**: 읽었다 / 못 읽었다 사이에 **읽었지만 그 숫자가
  전부가 아니다**가 있다. `task_cost_usd`가 돌려주는 `(합, 호출 수, 가격 미상 호출 수)`에서
  셋째를 버렸더니 부분합이 전체합처럼 읽혔고, 결말은 조회 실패를 0으로 접은 것과 같았다.
  **집계 함수가 수를 여럿 개 돌려주면 그중 버리는 것이 무엇을 숫자로 만드는지** 볼 것.
- **원장의 값을 줄이려면 그 돈을 못 쓰게 하는 권위도 함께 줄여야 한다.** Fleet 합계 예약을
  승인 카드 금액으로 줄였더니 상한이 깨졌다 — 합계 원장은 Rust에 있고 **실제로 쓸 수 있는
  금액을 정하는 것은 sidecar의 `TaskBudget`**이라, 원장만 줄이면 줄인 만큼은 "안 쓴다"는
  근거가 없는 금액이 된다(합계 $8 / 태스크당 $2에서 실제 지출 $12). 태스크 원장에서 같은
  설계가 성립했던 이유는 거기서는 **권위와 예약이 같은 객체**이기 때문이다. 근거:
  state-machine 72.12.1절.
